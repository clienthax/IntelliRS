...
import com.galkon.node.NodeSub;
...
public class DrawingArea extends NodeSub {
	...
	/**
	 * Draws a gradient from top to bottom.
	 * @param x
	 * @param y
	 * @param w
	 * @param h
	 ...
	 * @param endColor
	 */
	public static void drawVerticalGradient(int x, int y, int w, int h, int startColor, int endColor) {
		int position = 0;
		int scale = 0x10000 / h;
		if (x < startX) {
			w -= startX - x;
		...
		int offset = width - w;
		int pixel = x + y * width;
		for (int indexY = -h; indexY < 0; indexY++) {
			int unknown1 = 0x10000 - position >> 8;
			int unknown2 = position >> 8;
			int value = ((startColor & 0xff00ff) * unknown1 + (endColor & 0xff00ff) * unknown2 & 0xff00ff00) + ((startColor & 0xff00) * unknown1 + (endColor & 0xff00) * unknown2 & 0xff0000) >>> 8;
			for (int indexX = -w; indexX < 0; indexX++) {
				pixels[pixel++] = value;
			}
			pixel += offset;
			position += scale;
	...
	/**
	 * Draws a gradient from top to bottom with the specified alpha value.
	 * @param x
	 * @param y
	 * @param w
	 * @param h
	 ...
	 * @param alpha
	 */
	public static void drawVerticalGradient(int x, int y, int w, int h, int startColor, int endColor, int alpha) {
		int position = 0;
		int scale = 0x10000 / h;
		if (x < startX) {
			w -= startX - x;
		...
		for (int _y = -h; _y < 0; _y++) {
			int gradient1 = 0x10000 - position >> 8;
			int gradient2 = position >> 8;
			...
			for (int _x = -w; _x < 0; _x++) {
				int colored_pixel = pixels[pixel];
				colored_pixel = ((colored_pixel & 0xff00ff) * opacity >> 8 & 0xff00ff) + ((colored_pixel & 0xff00) * opacity >> 8 & 0xff00);
				pixels[pixel++] = color + colored_pixel;
			}
			pixel += offset;
			position += scale;
		}
