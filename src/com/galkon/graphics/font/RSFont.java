package com.galkon.graphics.font;
...
public final class RSFont extends DrawingArea {
	...
	public RSFont(boolean flag, String name, Archive archive) {
		...
		strikeThrough = false;
		this.name = name;
		JagexBuffer font = new JagexBuffer(archive.getFile(name + ".dat"));
		JagexBuffer index = new JagexBuffer(archive.getFile("index.dat"));
	...
	public void drawShadowedString(String string, int x, int y, int color, boolean shadow) {
		...
		if (strikeThrough) {
			DrawingArea.drawHorizontalLine(lineX, y + (int) ((double) baseHeight * 0.69999999999999996D), x - lineX, 0x800000);
		}
	}
	...
	private void drawCharacter(byte charPixels[], int x, int y, int width, int height, int color) {
		...
		if (y < DrawingArea.startY) {
			int offsetY = DrawingArea.startY - y;
			height -= offsetY;
		...
		if (x < DrawingArea.startX) {
			int offsetX = DrawingArea.startX - x;
			width -= offsetX;
	...
	private void drawAlphaCharacter(byte pixels[], int x, int y, int alpha, int k, int i1, int j1) {
		...
		if (y < DrawingArea.startY) {
			...
			y = DrawingArea.startY;
			j2 += k2 * k;
			offset += k2 * DrawingArea.width;
		}
		if (y + i1 >= DrawingArea.endY)
			i1 -= ((y + i1) - DrawingArea.endY);
		if (x < DrawingArea.startX) {
		...
		if (x + k >= DrawingArea.endX) {
			int i3 = ((x + k) - DrawingArea.endX);
			k -= i3;
			i2 += i3;
			l1 += i3;
		}
		if (k <= 0 || i1 <= 0)
			return;
		createAlphaPixels(pixels, i1, offset, DrawingArea.pixels, j2, k, i2, l1, j1, alpha);
	}
