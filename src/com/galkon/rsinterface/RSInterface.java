...
import com.galkon.node.MemCache;
import com.galkon.util.DataUtils;
import com.galkon.util.TextUtils;
...
public class RSInterface {
	...
	public static void load(Archive interfaces, Archive media, RSFont fonts[]) {
		...
		while (buffer.offset < buffer.payload.length) {
			RSInterface rsi = DataChunk.readHeaderChunk(buffer);
			DataChunk.readTypeChunk(rsi, buffer);
		}
		spriteNodes = null;
	}
	...
	public static byte[] getData() {
		try {
			...
			for (int index = 0; index < cache.size(); index++) {
				...
				buffer.write(DataChunk.getHeaderChunk(rsi));
				buffer.write(DataChunk.getTypeChunk(rsi));
			}
			buffer.close();
