...
public final class RSImage extends RSDrawingArea {
	...
	public void drawARGBImageOLD(int xPos, int yPos, int alpha) {
		...
		if (!(spriteWidth <= 0 || spriteHeight <= 0)) {
			setARGBPixels(spriteWidth, spriteHeight, myPixels, RSDrawingArea.pixels, i1, alphaValue, j1, j2, i2);
		}
	}
	...
	public void drawARGBImage(int x, int y, int alpha) {
		x += this.offsetX;
		y += this.offsetY;
		int destOffset = x + y * RSDrawingArea.width;
		int srcOffset = 0;
		int height = this.myHeight;
		int width = this.myWidth;
		int destStep = RSDrawingArea.width - width;
		int srcStep = 0;
		if (y < RSDrawingArea.startY) {
			int trimHeight = RSDrawingArea.startY - y;
			height -= trimHeight;
			y = RSDrawingArea.startY;
			srcOffset += trimHeight * width;
			destOffset += trimHeight * RSDrawingArea.width;
		}
		if (y + height > RSDrawingArea.endY) {
			height -= (y + height) - RSDrawingArea.endY;
		}
		if (x < RSDrawingArea.startX) {
			int trimLeft = RSDrawingArea.startX - x;
			width -= trimLeft;
			x = RSDrawingArea.startX;
			srcOffset += trimLeft;
			destOffset += trimLeft;
		...
		if (x + width > RSDrawingArea.endX) {
			int trimRight = (x + width) - RSDrawingArea.endX;
			width -= trimRight;
			srcStep += trimRight;
			destStep += trimRight;
		}
		if (!((width <= 0) || (height <= 0))) {
			setARGBPixels(width, height, RSDrawingArea.pixels, myPixels, alpha, destOffset, srcOffset, destStep, srcStep);
		}
	}
