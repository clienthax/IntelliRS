...
public class RSInterface {
	...
	public static void load(Archive interfaces, Archive media, RSFont fonts[]) {
		...
		JagexBuffer buffer = new JagexBuffer(interfaces.getFile("data"));
		int parentId = -1;
	...
	public static void save() {
		try {
			JagexBuffer buffer = new JagexBuffer(new byte[999999]);
			...
			out.write(buffer.payload, 0, buffer.offset);
			out.close();
			JOptionPane.showMessageDialog(null, "Interfaces saved successfully!", "Success!", JOptionPane.INFORMATION_MESSAGE);
		} catch (IOException e) {
